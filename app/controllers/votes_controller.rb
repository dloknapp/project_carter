class VotesController < ApplicationController
  ...
  def index
    ...
    @votes = @q.result(:distinct => true).includes(:user, :image).page(params[:page]).per(10)
  ...
  def create
    ...
    @vote.yes_vote = params[:yes_vote]
    @vote.no_vote = params[:no_vote]
    @vote.user_id = params[:user_id]
    @vote.image_id = params[:image_id]
